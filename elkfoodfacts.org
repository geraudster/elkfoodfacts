...
1. Récupérer les submodules: =git submodule init= et =git submodule update=
2. Initialiser les credentials: =docker-compose -f setup.yml up=
3. Penser à noter le mot de passe pour l'utilisateur =elastic=
4. Lancer elasticsearch: =docker-compose up -d elasticsearch=
5. Lancer kibana: =docker-compose up -d kibana=
6. Ouvrir http://localhost:5601
7. Aller dans _Dev Tools_
...
*** Installation Docker 				       :B_alertblock:
:PROPERTIES:
:BEAMER_env: alertblock
:END:

Pour l'utilisation sous Docker, il vous faudra en plus configurer les propriétés suivantes :
- hosts = ["elasticsearch"]
- user = "elastic"
- password = "changeme"
